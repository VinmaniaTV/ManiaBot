...
exports.command = {
    ...
    execute: async (interaction) => {
        const youtubeUrl = interaction.options.get('youtubeurl')?.value;
            ...
            await interaction.editReply({
                embeds: [
                    new discord_js_1.EmbedBuilder()
                        ...
                        .addFields({ name: 'Durée', value: `${Math.floor(song.duration / 60)}:${song.duration % 60}` }, { name: 'Demandé par', value: song.requester })
                ]
            });
        }
        else {
